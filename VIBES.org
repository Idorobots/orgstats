...
** TODO Color & other adjustments                                                           :UX:TUI:
...
** TODO Default FILE to *.org in cwd                                                        :UX:CLI:
...
** TODO Stats command                                                                 :CLI:Analysis:
...
#+begin_src bash
orgstats stats tags -n 5 path/to/file.org
#+end_src
...
#+begin_src bash
orgstats stats tags --show Scala,Debugging,EmbeddedDevelopment path/to/file.org
#+end_src

The output generated should include only the stats for ~Scala~, ~Debugging~ and ~EmbeddedDevelopment~.

** TODO Groups command                                                                      :CLI:UX:
Add a new ~stats tags~ command to the CLI that allows displaying tag stats for specific tags.
An example invocation will look like this:

#+begin_src bash
orgstats stats groups -n 5 path/to/file.org
#+end_src
...
#+begin_src bash
orgstats stats groups --group Scala,Debugging,Testing --group Erlang,OTP path/to/file.org
#+end_src
...
#+begin_src bash
orgstats tasks --filter-completed --filter-category simple path/to/file.org
#+end_src
...
Empty result:
#+begin_example
orgstats tags --filter-not-complete VIBES.org
#+end_example
...
Results found:
#+begin_example
orgstats tags --filter-complete VIBES.org
#+end_example
...
Example:
#+begin_example
orgstats tags -n 4 --details tests/fixtures/simple.org
#+end_example
