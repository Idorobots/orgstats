...
** TODO Separate limit switches for tags, groups & tasks                                    :CLI:UX:
...
*** TODO Top relations limit
Change the semantics of the ~--max-relations~ switch, so that it default sto 5. The switch should accept values >= 0, and if it is not provided, omit those sections from tags display.

It is completely OK if there are fewer results than the limit. Do show the section title and "No results" in such a case (unless the user requested 0 results).

** TODO Task "category" histogram                                                     :Analysis:TUI:
...
** TODO Per-tag stats                                                                 :Analysis:TUI:
...
** TODO Per-group stats                                                               :Analysis:TUI:
...
** TODO Consistent timerange an all charts
...
** Stats command
Wrap the current analysis logic into a dedicated ~stats~ command. There will be more commands to come.
...
The rest should be accepted by the ~stats~ command.
