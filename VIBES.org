...
** TODO Display inconsistencies fixes                                                       :UX:Bug:
...
- --filter-not-completed should also include tasks withuout a todo state (so, task state in todo keys or absent).
